"""
UAV Mission 1 - Webots Simulation Edition (Elite v3.0)
======================================================
...
def perform_landing(drone, tracker):
    ...
    while time.time() - land_start < 40:
        ...
        if frame is not None:
            ...
            cv2.imshow("Mavic 2 PRO - Downward Camera HD", frame)
            cv2.waitKey(10)
...
# ============================================================
def main():
    log_event("--- SIMULATION MISSION 1 STARTING ---")
    ...
    bridge.send_message("UAV SIMULATION MISSION 1 LIVE")
    ...
    # === TRACKING LOOP ===
    log_event("Signaling UGV to begin Mission 1 Route...")
    bridge.send_command(cmdSeq=1, cmd=v2v_bridge.CMD_MISSION_1, payload=0)
    ...
    while True:
        ...
        if result is not None:
            # Sticky Lock Logic
            searcher.reset()
            is_centered = abs(result.dx) < 80 and abs(result.dy) < 80
            ...
            if not ugv_stopped:
                log_event("[V2V] Marker spotted! Braking UGV.")
                bridge.send_command(cmdSeq=seq, cmd=v2v_bridge.CMD_STOP, payload=0)
                ugv_stopped = True
                seq += 1

            # Elite Centering Deadzone: Eliminate the 3m wobble
            vx_raw = 0.0 if abs(result.dy) < 30 else (result.dy / 240.0)
            vy_raw = 0.0 if abs(result.dx) < 30 else (result.dx / 320.0)

            # High-Speed Pursuit PID Tuning
            vx = clamp(-vx_raw * 0.8, -1.2, 1.2)
            vy = clamp(vy_raw * 0.8, -1.2, 1.2)
            drone.set_velocity(vx, vy, 0.0) 
